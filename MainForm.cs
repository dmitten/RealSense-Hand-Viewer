...
namespace hands_viewer.cs
{
    ...
    public partial class MainForm : Form
    {
        ...
        public bool GetExtremitiesModeState()
        {
            return ExtremitiesMode.Checked;
        }
        ...
        public void DisplayCursor(int numOfHands, Queue<PXCMPoint3DF32>[] cursorPoints, int[] cursorClick)
        {
            if (bitmap == null) return;
            ...
            if (Cursor.Checked && CursorMode.Checked)
            {
                ...
                for (int i = 0; i < numOfHands; ++i)
                {
                    ...
                    if (0 < cursorClick[i] && cursorPoints[i].Count()>0)
                    {
                        color = Color.LightBlue;
                        pen = new Pen(color, 10.0f);
                        sz = 32;

                        int x = (int)cursorPoints[i].ElementAt(cursorPoints[i].Count - 1).x / scaleFactor;
                        int y = (int)cursorPoints[i].ElementAt(cursorPoints[i].Count - 1).y / scaleFactor;
                        g.DrawEllipse(pen, x - sz / 2, y - sz / 2, sz, sz);
                    }
                }
                pen.Dispose();
        ...
        private void TrackingModeChanged(object sender, EventArgs e)
        {
            ...
            if (ExtremitiesMode.Checked)
            {
                Labelmap.Enabled = true;
                Depth.Enabled = true;

                //Extremities.Enabled = true;
                Extremities.Checked = true;
            }
